    ...
    try {
        ...
        const emiQuery = `
            SELECT
                COALESCE(SUM(
                    CASE
                        WHEN payment_type IS NULL OR payment_type = 'EMI'
                        THEN emi_amount
                        ELSE 0
                    END
                ), 0) AS total_emi_paid,
        ...
        const activeLoanQuery = `
            SELECT
                ...
                COUNT(
                    CASE
                        WHEN e.payment_type IS NULL OR e.payment_type = 'EMI'
                        THEN 1
                    END
                ) AS paid_emis,
                GREATEST(
            ...
            WHERE l.user_id = $1
              AND l.status = 'active'
            GROUP BY
                l.id, l.bank_name, l.total_loan_amount, l.emi_amount,
                l.total_emis, l.next_emi_date, l.status
        ...
        const activeLoans = activeLoanResult.rows.map(row => ({
            ...
            total_emis: parseInt(row.total_emis, 10) || 0,
            paid_emis: parseInt(row.paid_emis, 10) || 0,
            remaining_emis: parseInt(row.remaining_emis, 10) || 0,
            next_emi_date: formatDate(row.next_emi_date),
            days_until_next_emi: parseInt(row.days_until_next_emi, 10) || 0,
            status: row.status,
        ...
        const activeBorrowQuery = `
            SELECT
                ...
                GREATEST(
                    b.borrow_amount - COALESCE(SUM(r.repayment_amount), 0),
                    0
                ) AS remaining_amount,
            ...
            WHERE b.user_id = $1
              AND b.status = 'active'
            GROUP BY
                b.id, b.person_name, b.borrow_amount,
                b.take_date, b.return_date, b.status
        ...
        const activeBorrows = activeBorrowResult.rows.map(row => ({
            ...
            borrow_amount: parseFloat(row.borrow_amount) || 0,
            total_repaid: parseFloat(row.total_repaid) || 0,
            ...
            days_remaining: parseInt(row.days_remaining, 10) || 0,
            status: row.status
        }));
        ...
        const quickStatsQuery = `
            SELECT 
                ...
                (SELECT COALESCE(SUM(borrow_amount), 0)
                 FROM personal_borrow
                 WHERE user_id = $1) AS all_borrow,
                (SELECT COALESCE(SUM(total_loan_amount), 0)
                 FROM personal_loans
                 WHERE user_id = $1) AS all_loan,
                (SELECT COALESCE(SUM(
                    CASE
                        WHEN payment_type IS NULL OR payment_type = 'EMI'
                        THEN emi_amount
